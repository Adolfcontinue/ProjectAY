...
void ACharacterBase::RepPlayerMove(FVector pos, float yaw)
{
	CurrentLocation = GetActorLocation(); // 현재 위치 저장
	TargetLocation = pos;   // 서버로부터 받은 목표 위치 설정
	LerpAlpha = 0.0f;

	CurrentRotation2 = GetActorRotation();   // 현재 회전 저장
	TargetRotation2.Pitch = CurrentRotation2.Pitch;
	TargetRotation2.Yaw = yaw;
	TargetRotation2.Roll = CurrentRotation2.Roll;
	SlerpAlpha = 0.0f;
}
